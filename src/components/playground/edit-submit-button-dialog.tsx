...
import { CornerDownRight, Eye, RotateCcw, Settings2, Shuffle } from "lucide-react";
...
export function EditSubmitButtonDialog({
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      buttonLabel: formSpec.metadata.submitButtonLabel,
  ...
  function onSubmit(values: z.infer<typeof formSchema>) {
    ...
    setMetadata({
      ...formSpec.metadata,
      submitButtonLabel: values.buttonLabel,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[768px]">
        <DialogHeader>
          <DialogTitle className="mb-4 text-2xl leading-none tracking-tight">Customize submit button</DialogTitle>
        </DialogHeader>
        <Form {...form}>
              ...
              <div className="w-1/2 space-y-2">
                <div className="flex items-center gap-2">
                  <Button
                    ...
                    onClick={() => {
                      const randomColor =
                        Object.keys(tailwindColors)[Math.floor(Math.random() * Object.keys(tailwindColors).length)];

                      const shuffleShades = [300, 400, 500, 600, 700];
                      const randomShade = shuffleShades[Math.floor(Math.random() * shuffleShades.length)];
                  ...
                  <Button
                    disabled={
                      buttonLabel === "Submit" && buttonColor === "zinc" && buttonShade === 900 && fullWidth === false
                    }
                    size="sm"
                    type="button"
                    variant="outline"
                ...
                <Card className="flex flex-col items-center justify-center p-3">
                  <div className="flex items-center gap-1 self-start text-neutral-500 select-none">
                    ...
                    <span className="text-xs font-medium tracking-tighter uppercase">Preview</span>
                  </div>
                  ...
                  <Alert className="bg-gray-200">
                    ...
                    <AlertDescription className="text-xs">
                      Feel free to adjust the look and feel so it perfectly matches your brand and style.
                    </AlertDescription>
                  </Alert>
                </Card>
              </div>
              <div className="w-1/2 space-y-2 pt-11">
                <Card className="flex flex-col items-center justify-center gap-6 p-3">
                  <div className="flex items-center gap-1 self-start text-neutral-500 select-none">
                    ...
                    <span className="text-xs font-medium tracking-tighter uppercase">Edit</span>
                  </div>
                        ...
                        <FormItem className="flex flex-row items-center justify-between gap-4 rounded-md border border-zinc-200 p-4">
                          <div className="space-y-1 leading-none">
                            ...
                            <FormDescription className="text-xs">
                              When set, the button will stretch to occupy full width.
                            </FormDescription>
                          </div>
                          <FormControl>
