...
export function FieldCardHeader({ fieldType, fieldFormat }: { fieldType: FieldType; fieldFormat: FieldFormat }) {
  const fieldSection = fieldSections.find((section) => section.type === fieldType);
    ...
    <div className="flex items-center gap-1.5">
      ...
      <div className="flex flex-col justify-center gap-1 pt-px">
        <span className="leading-none font-medium tracking-tight">
          {isTruthy(fieldSection?.sectionName) ? fieldSection.sectionName.slice(0, -1) : "Unknown field"}
        </span>
        <span className="text-sm leading-none tracking-tight text-zinc-500">
          {isTruthy(fieldFormatData?.label) ? fieldFormatData.label : "Unknown format"}
        </span>
      </div>
    </div>
  );
