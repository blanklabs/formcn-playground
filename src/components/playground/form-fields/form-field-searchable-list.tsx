...
import { Field, FieldType, FieldTypeSchema } from "@/core/types";
import { usePlaygroundStore } from "@/stores/playground";
import { fieldSections } from "@/core/static/field-sections";

export function FormFieldSearchableList({ closeDialog }: { closeDialog: () => void }) {
      ...
      <div className="mb-4 flex flex-col gap-3 md:flex-row md:px-6">
        ...
        <div className="flex gap-2 overflow-x-scroll pl-4 last:pr-4 md:order-1 md:overflow-x-visible md:pl-0 md:last:pr-0">
          <div className="flex gap-2" key="filter-types">
            {FieldTypeSchema.options.map((fieldType) => (
              <Button
                key={fieldType}
                ...
                className={cn(
                  ...
                  filters.includes(fieldType) && "bg-amber-400 hover:bg-amber-400/90",
                )}
              >
      ...
      <div className="flex-grow space-y-6 last:pb-6">
          ...
          .map((fieldSection) => (
            <div key={fieldSection.type}>
              <div className="mb-4 flex items-center gap-2 px-4 md:px-6">
                ...
                >
                  {fieldSection?.icon ? <fieldSection.icon className="h-5 w-5" /> : <Square className="h-5 w-5" />}
                </div>
                <h3 className="text-xl leading-none font-semibold tracking-tight">{fieldSection.sectionName}</h3>
              </div>
        ...
        {fieldSections.filter(
          ...
          <div className="flex h-full flex-col items-center justify-center gap-5 text-zinc-500">
            ...
            <p className="text-center tracking-tight">No matching fields found. Try a different search term.</p>
          </div>
        )}
      </div>
    </div>
