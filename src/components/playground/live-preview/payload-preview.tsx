...
export function PayloadPreview({ className }: { className?: string }) {
  const { payloadPreview: formattedCode, setPayloadPreview } = usePlaygroundStore();
  ...
  return (
    <div>
      <h2 className="mb-0.5 text-xl font-bold tracking-tight">Request body preview</h2>
      <p className="mb-4 text-sm text-zinc-500">
        When your form gets submitted, these values will be sent to the server. Hit the submit button to refresh them!
      </p>
      <div className={cn("relative mb-2 min-h-48 overflow-hidden rounded-xl bg-[#090b0e]", className)}>
        <div className="max-h-96 overflow-y-scroll p-4 text-xs" dangerouslySetInnerHTML={{ __html: html }} />
        <div className="absolute top-0 right-0 pt-4 pr-3.5">
          ...
          >
            {isCopied ? <Check size={14} className="text-zinc-100" /> : <Copy size={14} className="text-zinc-100" />}
          </button>
        </div>
      </div>
      <div>
